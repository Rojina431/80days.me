...
import FirstChat from './chatcomp'
import SecondChat from './hicomp'
import EachChat from './eachchat'

const ChatComponent = () => {

    const [isOpen, setIsOpen] = useState(false)
    const [comp, setComp] = useState("first")
    ...
    return (
            ...
            {isOpen && <Fragment>{comp === "first" ? <FirstChat handleRedirect={handleRedirect}/> : 
            comp === "second" ? <SecondChat handleRedirect={handleRedirect}/> : <EachChat handleRedirect={handleRedirect}/>}</Fragment>}
            {!isOpen ? <div className='chat-btn' onClick={() => setIsOpen(true)}>
